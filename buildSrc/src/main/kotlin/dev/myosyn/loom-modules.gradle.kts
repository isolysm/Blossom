...
import dev.myosyn.gradle.multiversion.Platform
...
revisions.add(Revision(
    ...
    // Quilt mappings can be interchanged with yarn I think?
    // This is only because of the fact that it is literally just yarn but with tweaked rendering things,
    mp = mapOf(
        11802 to "22",
        // There are no other Quilt mappings for older versions, so great.
    ),
...
dependencies {
    ...
    val mappingsStr = prop("mappings", when {
        platform.isQuilt ->
            revision.mp[platform.mcVersion]?.let {"org.quiltmc:quilt-mappings:$it"}
        platform.isFabric ->
    ...
    if (platform.isFabric) {
        ...
        modImplementation(prop("quilt-loader", "org.quiltmc:quilt-loader:${revision.quiltLoader}"))
    }
    else {
